...
if cart_items:
    ...
    # ========== SECTION 7: SURVEY DATA INSIGHTS ==========
    st.header("7️⃣ 📈 Research Insights")
    st.markdown("<p style='color: #888; font-size: 14px;'>Powered by AI analysis of 25 survey responses + 500+ online reviews</p>", unsafe_allow_html=True)
    
    tab1, tab2 = st.tabs(["Category Behavior", "Discovery Preferences"])
    
    with tab1:
        st.image("survey_insights_overview.png", use_container_width=True)
        st.markdown("""
        <div style='background-color: #FFF3E0; padding: 12px; border-radius: 8px; margin-top: 10px;'>
            <b>🔍 Key Finding:</b> 68% of users buy only Snacks & Beverages. Non-grocery categories (Beauty, Electronics, Pet) have <15% penetration. 
            The grocery-only mental model is deeply entrenched.
        </div>
        """, unsafe_allow_html=True)
    
    with tab2:
        st.image("discovery_features_triggers.png", use_container_width=True)
        st.markdown("""
        <div style='background-color: #E8F5E9; padding: 12px; border-radius: 8px; margin-top: 10px;'>
            <b>💡 Key Finding:</b> 40% of users want <b>Mystery Free Samples</b> at ₹500+ basket. 
            Low-risk trial mechanisms outperform traditional discovery features by 2x.
        </div>
        """, unsafe_allow_html=True)
